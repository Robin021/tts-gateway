...
服务地址(默认):`221.194.152.20:8000`

| 想做什么 | 用哪个接口 |
|---|---|
| **给已有项目加 TTS,不想动太多代码** | HTTP `POST /v1/audio/speech`(OpenAI 兼容) |
| **做对话 / 实时 / 客户能打断** | WebSocket `/ws/tts` |
| **看接口文档(Swagger UI)** | `http://221.194.152.20:8000/docs` |
| **机器可读 OpenAPI schema** | `http://221.194.152.20:8000/openapi.json` |
...
```bash
curl -X POST http://221.194.152.20:8000/v1/audio/speech \
...
client = OpenAI(
    api_key="YOUR_TOKEN",
    base_url="http://221.194.152.20:8000/v1",
)
...
with httpx.stream(
    "POST",
    "http://221.194.152.20:8000/v1/audio/speech",
...
- URL: `ws://221.194.152.20:8000/ws/tts`
...
const ws = new WebSocket("ws://221.194.152.20:8000/ws/tts", "tts.v1");
...
```bash
curl http://221.194.152.20:8000/v1/audio/voices \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
- **Swagger UI**:`http://221.194.152.20:8000/docs`
  可以填表单 / 直接发请求,适合快速测
- **ReDoc**:`http://221.194.152.20:8000/redoc`
  适合阅读,排版好看
- **OpenAPI JSON**:`http://221.194.152.20:8000/openapi.json`
  生成 SDK 用
